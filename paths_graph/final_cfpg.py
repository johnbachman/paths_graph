...
class CFPG(PathsGraph):
    """
    The direct_from_pg method here handles the polarity = 0 case.
    """
    ...
    * CF3: There is a 1-1 correspondence between the paths in G_cf and the
    ...
    def direct_from_pg(klass, pg):
        ...
        pg_raw = pg.graph
        ntp_0 = [v for v in pg_raw.nodes() if (v != src_2node and v[1] == src_2node[1]) or (v != tgt_2node and v[1] == tgt_2node[1])]
        ...
        past = Past(src_2node,tgt_2node, pg_0)
        next_tgt = {tgt_3node: []}
        ...
        # included in the set of tags of v
            for x in V_current:
                ...
                V_x, next_x, pred_x, t_cf_x = \
                         split_graph(src_2node, tgt_2node, x,  X_ip1, X_im1, t_cf_ip1, tags_x, pg_0)
                V_i.extend(V_x)
    ...
    def from_pre_cfpg(klass, pre_cfpg):
        ...
        dic_CF[0] = (V_0, next_src, pred_src, t_cf_src)
        G_cf = _dic_to_graph_pre_cfpg(dic_CF, pre_cfpg)
        # Prune out possible unreachable nodes in G_cf
...
def split_graph(src, tgt, x,  X_ip1, X_im1, t_cf, tags_x, g):
...
def _dic_to_graph_pre_cfpg(dic, pre_cfpg):
