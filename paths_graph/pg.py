...
class PathsGraph(object):
    ...
    def _get_cf_path_counts(self, total_path_counts):
        # Gets path counts using the cycle-free blacklist dict.
        """Get the number of cycle-free paths through each node.
        ...
        paths in the graph.
        """
        cf_counts = deepcopy(total_path_counts)
        # Iterate over the entries in the blacklist
        for path, blacklist_nodes in self._blacklist_by_path.items():
            for bn in blacklist_nodes:
                forward_count = total_path_counts[bn]
                for path_node in path:
                    cf_counts[path_node] = (cf_counts[path_node] -
                                            forward_count)
        return cf_counts
